...
import ModalEstudiante from "../../components/Admin/Estudiante/ModalEstudiante";

// Notification
import { showToast } from "../../components/Common/notifications";
import { getErrorMessageEstudiante } from "../../components/Common/errorMessage";
...
class DatatableTables extends Component {
  ...
  async createEstudiante (values) {
    try {
      delete values.id;
      this.setState({loadingForm:true});
      await helpAPI.post(`${import.meta.env.VITE_APP_BACKEND_URL}/estudiantes`, {...values })
      console.log("creado correctamente");
      showToast({message:this.props.t("Estudiante created successfully")});
      this.tog_xlarge();
      const state = this.tableRef.current.getNewestState();
      this.loadData(state)
    } catch (error) {
      console.log('error', error.response.data.error)
      showToast({toastType:'error',title:"Error",message:getErrorMessageEstudiante(error?.response?.data.error, this.props.t)})
    } finally {
      this.setState({loadingForm:false});
    }
  }
  async updateEstudiante (values, id){
    try {
      delete values.id;
      await helpAPI.put(`${import.meta.env.VITE_APP_BACKEND_URL}/estudiantes/${id}`, { ...values })
      this.setState({loadingForm:true});
  ...
  tog_xlarge() {
    this.setState(prevState => ({
      ...
      estudiante: !prevState.modal_xlarge ? prevState.estudiante : {id:null}
    }))
    this.removeBodyCss()
  }
  ...
  handleSubmit(value, id){
    if(!id) return this.createEstudiante(value)
    return this.updateEstudiante(value,id)
  }
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        sort: true,
        formatter: (cell, row) => row.usuario.identificacion || this.props.t("Estudiante does not has email"),
      },
      {
        dataField: "correo",
        text: this.props.t("Login email"),
        formatter: (cell, row) => row.usuario.correo || this.props.t("Estudiante does not has email"),
        sort: true,
      },
      {
        dataField: "codEstudiante",
        text: "Codigo Estudiante",
        ...
        <React.Fragment>
          <div className="page-content">
            <div className="container-fluid">
              <Breadcrumbs title={this.props.t("Administrator")} breadcrumbItem={this.props.t("Estudiantes")} />
          ...
          <ModalEstudiante
              estudianteData={this.state.data}
